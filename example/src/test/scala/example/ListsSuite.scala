...
class ListsSuite extends FunSuite {
  ...
  test("max of a few numbers") {
    ...
    assert(max_fold(List(3, 7, 2)) === 7)
  }

  test("max of negative numbers is the closest to 0") {
    assert(max(List(-10, -4, -20, -1)) === -1)
    assert(max_fold(List(-10, -4, -20, -1)) === -1)
    assert(max(List(-10, 0, -20, -1)) === 0)
    assert(max_fold(List(-10, 0, -20, -1)) === 0)
  }

  test("max of an empty list throws a NoSuchElementException") {
  ...
  test("max of the same numbers is the same number") {
    assert(max(List(4, 4, 4, 4)) === 4)
    assert(max_fold(List(4, 4, 4, 4)) === 4)
  }
}
